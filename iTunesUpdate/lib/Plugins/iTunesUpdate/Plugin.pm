...
my $ITUNES_UPDATE_PERCENT_PLAY_THRESHOLD = .01;
my $ITUNES_UPDATE_TIME_PLAY_THRESHOLD = 5;
...
sub webPages {
	...
	if (Slim::Utils::PluginManager->isEnabled("Plugins::iTunesUpdate::Plugin")) {
		...
		Slim::Web::Pages->addPageFunction("plugins/iTunesUpdate/index.html", \&handleWebIndex);
	} else {
...
sub cliQuery {
	...
	push @menuItems, {
	 	...
	 	input => {
	 		...
	 		help         => {
                ...
                'icon-id' => "/html/images/playlistsave.png",
	...
	if (my $playStatus = getTrackInfo($client)) {
		...
		if ($playStatus->checkediTunes() eq 'true') {
			...
			push @menuItems, { 
				text => $client->string('PLUGIN_ITUNES_UPDATER_NOT_FOUND'),
			       	style     => 'itemNoAction'
			};
		}
	} else {
		push @menuItems, { 
			text => $client->string('PLUGIN_ITUNES_UPDATER_NO_TRACK'), 
			name => $client->string('PLUGIN_ITUNES_UPDATER_NO_TRACK'), 
			style     => 'itemNoAction',
			hasitems => 0,
			isaudio => 0,
		};
	...
	$request->addResult('title',Slim::Utils::Strings::string('PLUGIN_ITUNES_UPDATER'));;
...
sub openCommand($$$)
{
	...
	my $previous = $playStatus->currentTrackOriginalFilename();
	...
	if ($prefs->get('bookmarks') and $playStatus->bookmark()){
		#check filename as this can trigger another open command?
		if ($previous ne $playStatus->currentTrackOriginalFilename()){
			$log->info("Pushing ".$client->id." to ".$playStatus->bookmark()."s");
			$client->execute(['time',$playStatus->bookmark()]);
		} else {
			$log->info("Ignoring bookmark!");
		}
	}
}
	...
	if ($trackHandle) {
		if ($action eq 'played') {
			...
			if ($os eq 'win') {
                ...
                my @t = localtime(time);
                my $offset = (timegm(@t) - timelocal(@t))/60;
                my $dbid =  $trackHandle;
                my $cmd = "osascript -e 'tell app \"iTunes\" to set played date of first track of playlist \"Library\" whose database ID is $dbid to current date + ($offset * minutes) '";

                system($cmd);
                
                my $playcount = `osascript -e 'tell app \"iTunes\" to get played count of first track of playlist \"Library\" whose database ID is $dbid'` +1;
                
                $cmd = "osascript -e 'tell app \"iTunes\" to set played count of first track of playlist \"Library\" whose database ID is $dbid to $playcount'";
                system($cmd);
			...
			if ($os eq 'win') {
                ...
                my $dbid =  $trackHandle;
                my $skippedcount = `osascript -e 'tell app \"iTunes\" to get skipped count of first track of playlist \"Library\" whose database ID is $dbid'` +1;
                my $cmd = "osascript -e 'tell app \"iTunes\" to set skipped count of first track of playlist \"Library\" whose database ID is $dbid to $skippedcount'";
                system($cmd);
			}
		}
		if ($playStatus->currentSongRated()){
...
sub _searchiTunes {
	...
	if ($os eq 'win') {
		return _searchiTunesWin($searchString, $fileLocation, $artist, $album, $title);
	} elsif ($os eq 'mac') {
		return _searchiTunesMac($searchString, $fileLocation, $artist, $album, $title);
	}
	return 0;
}
...
sub _searchiTunesMac {
    ...
    my $cmd = "osascript -e 'tell app \"iTunes\" to get database id of first track of playlist \"Library\" whose name is \"$title\"";

    $cmd .= " and artist is \"$artist\"" unless (!$artist or $artist eq string('NO_ARTIST'));
    $cmd .= " and album is \"$album\"" unless (!$album or $album eq string('NO_ALBUM'));
